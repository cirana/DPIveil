...
`drop_suspect_rst: true` deneysel ayarı, **yalnızca parçalanmış seçili alan adı bağlantılarında**, ClientHello gönderildikten sonraki 8 saniyede gelen ve kaydedilmiş SYN-ACK'e göre `ip_id=0 → ip_id≠0`, `TTL → TTL-1` örüntüsünü taşıyan RST paketini düşürür. Diğer RST'ler olduğu gibi geçer. Bu örüntü bir ağ cihazının müdahalesini düşündürebilir, fakat gönderenin kimliğini kanıtlamaz. Gerçek sunucunun RST'si de eşleşirse bağlantı sıfırlanmak yerine zaman aşımına uğrayabilir. Ayarı `false` yaparak anında kapatabilirsiniz. Başarılı HTTPS yanıtı alınmadan erişim sağlandı sayılmaz.
